...
| Pilot cohort | `report pilot-cohort --manifest <path> [--output <md|json>]` writes a local value-free design-partner cohort rollup from explicit pilot outcome packets without discovering cohorts, creating, importing, updating, syncing, assigning, labeling, commenting on, or posting to tickets, chat, automation, hosted state, PRs, dashboards, uploads, external APIs, Hurl/tests, model providers, traffic state, `entroping run`, source Hurl, raw outcome contents, design-partner private notes, URLs, or provider keys. |
...
| OpenTelemetry mapping | `report otel-mapping [--output <md|json>]` writes a value-free local OpenTelemetry evidence mapping packet from sanitized observability and test-evidence artifacts without exporting OTLP, calling collectors/vendor APIs, mutating dashboards/monitors/tickets/chat/PRs, reading provider keys, parsing traffic state, executing Hurl/tests, invoking models, or changing `entroping run`. |
| Observability adapter readiness | `report observability-adapter-readiness [--output <md|json>]` writes value-free local readiness rows for future OpenTelemetry, Datadog, Splunk, Grafana, and generic observability adapters without exporting OTLP, calling collectors/vendor APIs/webhooks, mutating dashboards/monitors/tickets/chat/PRs, reading provider keys, parsing traffic state, executing Hurl/tests, invoking models, or changing `entroping run`. |
| API inventory | `report api-inventory [--output <md|json>]` writes value-free local API style summaries for REST/OpenAPI, GraphQL, SOAP/XML, gRPC/proto, AsyncAPI, webhook/event, WebSocket/realtime, and unknown HTTP signals without generating tests or parsing traffic state. |
...
| `entroping report pilot-cohort --manifest <path> --output md` | `reports/pilot-cohort.md` |
| `entroping report pilot-cohort --manifest <path> --output json` | `reports/pilot-cohort.json` |
...
| `entroping report otel-mapping --output md` | `reports/otel-mapping.md` |
| `entroping report otel-mapping --output json` | `reports/otel-mapping.json` |
| `entroping report observability-adapter-readiness --output md` | `reports/observability-adapter-readiness.md` |
| `entroping report observability-adapter-readiness --output json` | `reports/observability-adapter-readiness.json` |
...
| `entroping report qa-brain-repair-plan --output md` | `reports/qa-brain-repair-plan.md` |
| `entroping report qa-brain-repair-plan --output json` | `reports/qa-brain-repair-plan.json` |
