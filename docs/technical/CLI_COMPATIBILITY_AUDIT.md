...
| Area | Decision |
| --- | --- |
| Command naming | Keep the nested noun/verb shape: `architect build`, `architect refactor`, `report bug`, `report traceability`. |
...
| Report formats | `run --report` is repeatable and owns run artifact creation. `report bug` and `report traceability` are handoff/reporting commands, not test execution commands. |

## Exit Code Contract

| Exit code | Meaning | Examples |
| --- | --- | --- |
| `0` | Successful command, successful run, or non-CI no-match run that is informational. | `--help`, `--version`, passing `run`, passing `report traceability`. |
| `1` | Runtime, configuration, report, Hurl, drift, or quality failure. | Invalid QAnstitution, missing Hurl, failing Hurl suite, drift finding with `--drift-check`, no failure available for `report bug`. |
| `2` | CLI usage or unsupported-mode error. | Unknown commands, unsupported `architect build --strategy`, unsupported `report traceability --output`, unsupported `run --report`. |
