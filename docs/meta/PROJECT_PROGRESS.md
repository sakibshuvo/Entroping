...
| [Factory status projection](https://github.com/sakibshuvo/Entroping/issues/1572) | Pending merge | Implementation-complete locally on the status branch: maintainer-only human/strict-JSON status, ordered exits `2/1/0`, bounded metadata-only reads, descriptor-pinned immutable SQLite, same-timestamp double collection, and observation-only threshold/reason diagnostics. |
