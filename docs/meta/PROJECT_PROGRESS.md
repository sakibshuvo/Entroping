...
| --- | --- | --- |
| [Factory Python3 compatibility and worker file-safety](https://github.com/sakibshuvo/Entroping/issues/658) / [#661](https://github.com/sakibshuvo/Entroping/issues/661) / [#659](https://github.com/sakibshuvo/Entroping/issues/659) | Done / Follow-up queued | Factory metrics, graph probe, AI queue, OpenCode worker, and DeepSeek worker entrypoints avoid evaluated Python 3.10+/3.11-only APIs under host `python3`; OpenCode/DeepSeek worker file selection now rejects sensitive credential-path variants before provider review; optional proxy-stack Tornado keeps a temporary `>=6.5.6` uv override until mitmproxy relaxes its cap. |
