...
| --- | --- | --- |
| [Portable factory role and metrics ledger](https://github.com/sakibshuvo/Entroping/issues/652) | Done | `docs/meta/AGENT_ROLE_REGISTRY.yaml` defines portable agent roles and authority limits, while `scripts/factory_metrics.py` records ignored local workflow metrics under `.entroping/factory-metrics/` without raw prompts, provider transcripts, secrets, raw traffic, product runtime evidence, or `entroping run` involvement. |
