...
| --- | --- | --- |
| [Scheduler crash and outage recovery](https://github.com/sakibshuvo/Entroping/issues/1571) | Done | Merged via PR #1597 at `b78c551a`; scheduler schema v3 adds versioned execution phases, heartbeat/phase/epoch fencing, deterministic bounded retry, ledger-first paid settlement authority, conservative legacy migration, and plan-first value-free recovery that never dispatches or releases uncertain holds. |
| [Factory status projection](https://github.com/sakibshuvo/Entroping/issues/1572) | Done | Maintainer-only human/strict-JSON status has ordered exits `2/1/0`, bounded metadata-only reads, descriptor-pinned immutable SQLite, same-timestamp double collection, and observation-only threshold/reason diagnostics. |
| [Tier A worktree orchestration](https://github.com/sakibshuvo/Entroping/issues/1574) | Done | Merged via PR #1600 at `ac5878a` on 2026-08-03. The maintainer-only adapter binds scheduler ownership and exact proposal bytes to one canonical issue worktree, allowlisted gates, a private lifecycle journal, and value-free revision receipts; it remains separate from PR/CI/merge authority. |
| [Offline proposal-controller validation](https://github.com/sakibshuvo/Entroping/issues/1575) | Evidence complete (offline) | The bounded 20-test harness emitted 32 strict schema-version-1 value-free receipts from fresh startup-guarded children, preserved tracked and relevant untracked source/Git state, and matched deterministic outcomes across repeated/concurrent runs. The final local pilot took 9.37s. It intentionally excludes live GitHub selection, provider dispatch, orchestration apply, and launchd. |
| [Tier A PR delivery controller](https://github.com/sakibshuvo/Entroping/issues/1576) | Focused local evidence; release gates pending | Local unmerged issue-branch implementation and independent review cover exact terminal receipt replay, local commit plus PR/CI/merge binding, strict finish partial replay, leased remote deletion with absence proof, fenced scheduler completion, post-lease completion without heartbeat/lease extension, and bounded deliver CLI output. Full security regression, quality audit, whole-branch review/QA, PR/CI, merge, and finish cleanup remain pending; launchd stays disabled and the product CLI/provider boundary is unchanged. |
| [Aggregate-PR finish cleanup](https://github.com/sakibshuvo/Entroping/issues/1549) | Implementation; release gates pending | A tracked PR #1537 evidence manifest now binds the 20 retained issue worktrees to exact source/integrated commits and stable patch IDs. The opt-in finish path verifies live issue/PR/CI/ancestry and clean source state before reusing strict local cleanup; no aggregate cleanup has been run here. |
| [Nonblocking QAnstitution enforcement and report evidence](https://github.com/sakibshuvo/Entroping/issues/1540) | Implemented | A single Hurl execution copy evaluates block, warn, and audit_only gates; source and block failures remain blocking while nonblocking failures stay observable in JSON, JUnit, and HTML. Structured Hurl 4.3 evidence is bounded, redacted, and scoped to gated paths; source Hurl bytes and normal ungated output remain unchanged. |
| [Provider scorecard evidence](https://github.com/sakibshuvo/Entroping/issues/1573) | Done | Maintainer-attested scorecard evidence binds registry identity, replay protection, per-case recency, cost correlation, and typed value-free reports while promotion remains manual. |
| [Provider quota and dispatch authorization](https://github.com/sakibshuvo/Entroping/issues/1570) | Done | Ledger schema v3 atomically binds cash thresholds, every quota dimension, a protected HMAC-authenticated provider-evidence envelope, immutable authority, exact terminal usage replay, and reset-aware accounting where timestamp order never proves provider coverage and only signed explicit included-authorization ids prevent settlement double counting; scheduler persistence and threshold-aware single-use launch complete the boundary. |
...
| [Unattended OpenCode isolation](https://github.com/sakibshuvo/Entroping/issues/1566) | Done | Pure fixed-agent workers use private ephemeral HOME/XDG roots, typed deny-first effective-config preflight, executable/profile binding, tool-free explicit-attachment review/patch profiles, and value-free capability receipts. |
