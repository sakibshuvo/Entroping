...
| [Lossless decision registry](https://github.com/sakibshuvo/Entroping/issues/468) / [#602](https://github.com/sakibshuvo/Entroping/issues/602) | Done | `docs/meta/DECISION_REGISTRY.yaml` indexes durable decisions, `scripts/context_pack.sh` surfaces the registry for agent handoff, and the Graphify pilot kept graph retrieval optional after proving `rg` plus context packs still beat natural-language graph queries for initial discovery while symbol-known graph impact checks can help maintainers. |
