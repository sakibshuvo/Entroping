...
| --- | --- | --- |
| [Captured traffic body Hurl-syntax hardening](https://github.com/sakibshuvo/Entroping/issues/671) | Done | Traffic-to-Hurl freeze generation now treats captured request bodies that look like Hurl structure or Entroping metadata as inert `base64,...;` body data, with bridge and freeze regressions proving injected body text cannot create extra parsed exchanges or metadata. |
| [Factory Python3 compatibility and worker file-safety](https://github.com/sakibshuvo/Entroping/issues/658) / [#661](https://github.com/sakibshuvo/Entroping/issues/661) / [#663](https://github.com/sakibshuvo/Entroping/issues/663) / [#665](https://github.com/sakibshuvo/Entroping/issues/665) / [#659](https://github.com/sakibshuvo/Entroping/issues/659) | Done / Follow-up queued | Factory metrics, graph probe, AI queue, OpenCode worker, and DeepSeek worker entrypoints avoid evaluated Python 3.10+/3.11-only APIs under host `python3`; OpenCode/DeepSeek selected-file preflight rejects sensitive paths and secret-like content before provider review, and OpenCode reviews use vetted snapshots instead of live selected-file reads; optional proxy-stack Tornado keeps a temporary `>=6.5.6` uv override until mitmproxy relaxes its cap. |
| [Portable factory roles and opt-in metrics hooks](https://github.com/sakibshuvo/Entroping/issues/652) / [#654](https://github.com/sakibshuvo/Entroping/issues/654) / [#656](https://github.com/sakibshuvo/Entroping/issues/656) / [#667](https://github.com/sakibshuvo/Entroping/issues/667) | Done | `docs/meta/AGENT_ROLE_REGISTRY.yaml` defines portable agent roles and authority limits; `scripts/factory_metrics.py`, `scripts/context_pack.sh --record-factory-metrics`, `scripts/ai_jobs.py run-next --record-factory-metrics`, `scripts/opencode_worker.py`, and `scripts/deepseek_worker.py` record ignored local workflow metrics without raw prompts, provider transcripts, secrets, raw traffic, product runtime evidence, or `entroping run` involvement, and `scripts/factory_metrics.py report` exports sanitized per-issue cost/yield summaries for future factory extraction. |
