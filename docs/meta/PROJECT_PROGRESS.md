...
| [Runtime known-failure gate exceptions](https://github.com/sakibshuvo/Entroping/issues/336) | Done | `ignore_failures` now skips only matching Entroping-injected QAnstitution gates by exact test path and rule ID, while reports expose applied exception evidence | Keep exceptions temporary, expiring, and issue-linked |
...
| [Second realistic API fixture](https://github.com/sakibshuvo/Entroping/issues/194) | Done | Support-ticket fixture adds filtered list reads, required request headers, POST/PATCH mutations, OpenAPI, Hurl smoke tests, QAnstitution gates, and real-run report guidance | Keep examples varied so demos do not overfit checkout |
| [GraphQL and SOAP protocol fixtures](https://github.com/sakibshuvo/Entroping/issues/337) | Done | `examples/graphql-api/` and `examples/soap-api/` show protocol-specific assertions through Hurl-over-HTTP, local demo servers, and QAnstitution gates | Keep protocol examples as fixtures, not runtime engines |
...
| [Live demo readiness and Hurl install guidance](https://github.com/sakibshuvo/Entroping/issues/265) | Done | `scripts/live_demo_smoke.sh` now documents its HTTP readiness probe boundary and points missing-Hurl users to direct install guidance plus `scripts/demo.sh` | Keep Hurl as the only API assertion executor |
...
| [Reusable policy pack layout](https://github.com/sakibshuvo/Entroping/issues/201) | Done | `docs/technical/POLICY_PACK_LAYOUT.md` and `examples/policy-packs/api-baseline/` define the first pack shape without runtime registry behavior | Keep pack imports local, inspectable, and open-core aligned |
| [Policy-pack distribution and provenance path](https://github.com/sakibshuvo/Entroping/issues/312) | Done | `docs/technical/POLICY_PACK_DISTRIBUTION.md` defines local-first pack versioning, distribution modes, import verification, attribution, open-core/premium boundaries, minimum smoke evidence, and implementation follow-ups | Keep distributed packs as local inspectable QAnstitution imports before runtime |
| [Policy-pack provenance manifest validation](https://github.com/sakibshuvo/Entroping/issues/317) | Done | `scripts/policy_pack_smoke.py --strict` validates local pack source, license, supported Entroping range, evidence command, gate files, declared gate IDs, and final flags against loaded QAnstitution gates | Keep provenance validation local evidence, not registry authenticity or runtime manifest dependency |
| [Reusable policy-pack verification artifact](https://github.com/sakibshuvo/Entroping/issues/329) | Done | `scripts/policy_pack_smoke.py --pack <local-pack> --format json --strict` emits reusable `policy-pack-verification` evidence for arbitrary local pack directories, including attribution, entrypoint imports, final gates, and consumer examples | Keep verification local and attachable without adding registry, install, or runtime manifest behavior |
...
| [Traffic store schema version policy](https://github.com/sakibshuvo/Entroping/issues/262) | Done | SQLModel-backed `.entroping/state.db` now records `schema_version=1`, rejects future versions, and documents migration/read-only compatibility rules | Add reviewed migrations before changing traffic-store contracts |
| [Doctor traffic-state health](https://github.com/sakibshuvo/Entroping/issues/275) | Done | `entroping doctor` reports missing, readable, and incompatible `.entroping/state.db` state through the read-only traffic-store path without creating runtime state | Keep health checks local and non-mutating |
...
| [Local wheel install smoke](https://github.com/sakibshuvo/Entroping/issues/313) | Done | `scripts/local_wheel_install_smoke.py` installs the built wheel into a temporary venv, runs installed `entroping --version`, `init --minimal`, and `doctor`, and emits JSON/Markdown evidence without PyPI/TestPyPI | Keep package-index proof separate until Trusted Publishing runs |
| [Python compatibility policy](https://github.com/sakibshuvo/Entroping/issues/229) | Done | Package metadata and CI now claim/prove Python 3.12 and 3.13, including optional-extras smoke on both versions | Python 3.14 is not claimed until CI evidence is added |
...
| [Artifact-backed review summary](https://github.com/sakibshuvo/Entroping/issues/316) | Done | `entroping report review-summary` writes provider-neutral Markdown from local JSON, JUnit, drift, and optional traceability evidence to `reports/review-summary.md` | Keep posting/uploading provider-specific and outside the deterministic run path |
...
| [Read-only Studio applied-gate drilldowns](https://github.com/sakibshuvo/Entroping/issues/192) | Done | Studio links latest-run report rule IDs to QAnstitution gate definitions without running Hurl or editing tests/config | Keep reports as the durable evidence |
| [Read-only Studio traffic session browser](https://github.com/sakibshuvo/Entroping/issues/190) | Done | Studio reads redacted SQLModel-backed state, shows target/dependency grouping, route summaries, and safe redaction categories and counts | It does not start `watch` or render raw URLs with query values, headers, bodies, cookies, tokens, or secrets |
...
| [Focused CLI command modules](https://github.com/sakibshuvo/Entroping/issues/260) | Done | The Typer entrypoint now delegates to focused project, config, architect, execution, and report adapters while preserving the locked command surface | Keep adapter modules thin and CLI-only |
...
| [README use cases and animated previews](https://github.com/sakibshuvo/Entroping/issues/291) | Done | README now front-loads concrete developer use cases and embeds curated GIF previews for the checkout happy path and AI-regression failure proof | Keep public media reviewed, small, and backed by guard tests |
...
| [QAnstitution authoring schema](https://github.com/sakibshuvo/Entroping/issues/267) | Done | `docs/technical/qanstitution.schema.json`, VS Code mapping, and schema drift tests now support editor feedback while keeping runtime validation authoritative | Regenerate the schema when Pydantic models change |
| [Public repo surface hygiene](https://github.com/sakibshuvo/Entroping/issues/266) | Done | Obsidian machine state is local-only, the vault index lives under `docs/meta/VAULT_INDEX.md`, and `.context/` is explicitly retained as maintainer/agent handoff context | Keep the root focused on standard open-source entry points |
...
| [Downstream GitHub Actions starter](https://github.com/sakibshuvo/Entroping/issues/189) | Done | Copyable workflow installs tagged Entroping, verifies pinned Hurl, runs `entroping run --ci --report json --report junit --report html`, writes a review summary, and uploads reports | Keep starter aligned with current report paths and release tag |
| [Non-GitHub CI provider recipes](https://github.com/sakibshuvo/Entroping/issues/204) | Done | `docs/user/CI_PROVIDER_RECIPES.md` gives GitLab CI, Buildkite, CircleCI, and generic shell guidance while keeping untested native templates out of `examples/` | Promote provider-specific templates only after real runner proof |
...
| [PyPI/TestPyPI release path](https://github.com/sakibshuvo/Entroping/issues/186) | Done | TestPyPI-first Trusted Publishing runbook covers token-free GitHub Actions environments, PEP 440 alpha naming, preflight checks, PyPI publish policy, and yank/new-version rollback | Keep first upload blocked until package-index Trusted Publishers are configured |
| [PyPI/TestPyPI trusted publishing workflow](https://github.com/sakibshuvo/Entroping/issues/223) | Done | Manual `publish-python-package.yml` builds distributions without OIDC privileges, then publishes to protected `testpypi` or `pypi` environments through Trusted Publishing | Configure package-index Trusted Publishers before first upload |
| [Homebrew tap prototype](https://github.com/sakibshuvo/Entroping/issues/224) | Done | Formula template and runbook define the PyPI-sdist-based tap shape, required Hurl dependency, local audit/install/test smoke, and optional-extras exclusion | Promote only after a package-index alpha is published and smoke-tested |
...
| [Strict public docs deployment](https://github.com/sakibshuvo/Entroping/issues/228) | Done | PR CI runs `mkdocs build --strict`, and GitHub Pages publishes the curated `docs/` site from `main` | Keep Obsidian-only notes, source exports, and generated artifacts out of public navigation |
...
| [Standalone binary distribution decision](https://github.com/sakibshuvo/Entroping/issues/225) | Done | Standalone binaries remain deferred until PyPI alpha, Homebrew tap demand, release-owner signing/notarization runbooks, and platform support evidence justify the cost | Do not add Nuitka/PyInstaller automation before demand is proven |
...
| [Typed dependency-drift run errors](https://github.com/sakibshuvo/Entroping/issues/263) | Done | Dependency-drift observation failures now raise `DependencyDriftObservationError` under `RunWorkflowError` instead of a generic `ValueError` | Keep CLI output actionable while preserving typed core failures |
...
| [Organization QAnstitution import controls](https://github.com/sakibshuvo/Entroping/issues/202) | Done | ADR-0011 defines import provenance, override rules, final-gate behavior, offline validation, and effective-policy evidence before organization policy features are implemented | Build provenance/report evidence before remote registry behavior |
| [Effective QAnstitution policy evidence](https://github.com/sakibshuvo/Entroping/issues/279) | Done | `entroping report policy --output md|json` writes local effective-policy provenance for imports, overrides, final gates, and gate sources | Keep organization-policy claims tied to effective local evidence |
| [Policy-pack smoke evidence](https://github.com/sakibshuvo/Entroping/issues/287) | Done | `scripts/policy_pack_smoke.py --strict` validates the example API-baseline pack provenance manifest, entrypoint, imported gates, final gates, and consumer evidence through local QAnstitution imports | Keep policy-pack claims local and artifact-backed before registries or install commands |
| [Alpha launch-readiness evidence](https://github.com/sakibshuvo/Entroping/issues/288) | Done | `scripts/launch_readiness.py --strict` aggregates public demo, release, policy-pack, backlog, and stable-boundary evidence and runs from `scripts/release_check.sh` | Keep alpha launch readiness separate from v1 stable-core claims |
| [Demo proof matrix](https://github.com/sakibshuvo/Entroping/issues/289) | Done | `scripts/demo_matrix.sh --dry-run` shows the checkout happy path, AI-regression failure proof, policy-pack smoke, launch-readiness, and backlog-health proof commands | Use for launch rehearsal, not as a replacement for regression/security gates |
| [Public claims audit gate](https://github.com/sakibshuvo/Entroping/issues/280) | Done | `scripts/public_claims_audit.py` runs through documentation governance and blocks unsupported production/security claims in public Markdown | Add intentional claim exceptions only through docs governance review |
| [Dependency license policy gate](https://github.com/sakibshuvo/Entroping/issues/281) | Done | `docs/meta/dependency-license-policy.json` records reviewed direct dependency licenses, and `scripts/dependency_license_check.py` runs in the security feature gate | Review the policy file with every direct dependency change |
| [Downstream integration examples hardening](https://github.com/sakibshuvo/Entroping/issues/282) | Done | Guard tests keep GitHub Actions as the only committed provider-native template and require pinned Hurl, artifacts, and real-runner proof before adding other CI examples | Promote GitLab/Buildkite/CircleCI templates only after actual runner evidence |
| [AI-regression failure proof](https://github.com/sakibshuvo/Entroping/issues/283) | Done | `examples/ai-regression-demo/` plus `scripts/ai_regression_demo.sh` prove Entroping blocks a body-correct API that drops `X-Request-Id` | Keep launch demos showing both happy path and failure proof |
| [Stable-core readiness evidence check](https://github.com/sakibshuvo/Entroping/issues/284) | Done | `scripts/stable_core_readiness.py --strict` is part of release checks and reports alpha blockers without overclaiming v1 readiness | Treat stable-core as evidence-gated, not green-test-gated |
| [Stable-core blocker issue map](https://github.com/sakibshuvo/Entroping/issues/319) | Done | `scripts/stable_core_readiness.py --format json` exposes `blocker_issue_map`, linking each unresolved stable-core blocker to tracked GitHub issues #303 through #308 and #318 | Keep the map aligned with release-evidence blockers and GitHub issue state |
| [Stable-core release evidence ledger](https://github.com/sakibshuvo/Entroping/issues/293) | Done | `docs/meta/release-evidence.json` and `scripts/release_evidence.py --strict` record and validate alpha releases, last reviewed `main` CI, package-index status, and stable-core blockers | Keep the ledger current after releases, CI proof, and package-index changes |
| [Downstream smoke evidence harness](https://github.com/sakibshuvo/Entroping/issues/297) | Done | `scripts/downstream_smoke.py` creates an external temporary project, runs Entroping through the public CLI, and can copy JSON/JUnit/HTML evidence artifacts | Use a real external project before treating downstream user feedback as satisfied |
| [Release evidence Pages/downstream coverage](https://github.com/sakibshuvo/Entroping/issues/299) | Done | `scripts/release_evidence.py --strict` now validates Pages CI and records local downstream smoke evidence in the committed ledger | Keep maintainer-controlled smoke proof separate from package-index proof and real downstream feedback |
...
| [Studio Textual typing cleanup](https://github.com/sakibshuvo/Entroping/issues/264) | Done | Studio's lazy Textual adapter no longer uses `no_type_check`, and targeted mypy coverage stays green | Keep optional UI boundaries typed without importing Textual at module import time |
