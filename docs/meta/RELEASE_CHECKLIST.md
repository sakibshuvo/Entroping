...
- Confirm `watch` is described as capture-only, `freeze` is described as basic Hurl generation only, and `freeze --mock`, `map`, and `studio` are clearly presented as not built yet.
