...
## Current Issue Slice: #1146 QA Brain Routing Plan Blocker Summary

Fix QA Brain routing-plan summary aggregation so `summary.blockers_total`
counts unique blocker messages across routing rows. Keep the change narrow:
preserve row-level blocker lists, route counts, packet schema,
...
provider/model boundaries, traffic parsing, model routing, LiteLLM
