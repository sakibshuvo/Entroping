...
def test_readme_links_two_minute_launch_assets() -> None:
    ...
    assert "docs/assets/launch/terminal-demo-screenshot-set.md" in readme
    assert "docs/assets/launch/html-report-screenshot.svg" in readme
    assert "docs/assets/launch/dependency-map-example.md" in readme
    assert "scripts/live_demo_smoke.sh" in readme


def test_launch_asset_kit_is_text_first_and_reproducible() -> None:
    expected_files = {
        "README.md",
        "terminal-demo-screenshot-set.md",
        "html-report-screenshot.svg",
    ...
    html_screenshot = (LAUNCH_DIR / "html-report-screenshot.svg").read_text(
        encoding="utf-8"
    )
    ...
    assert "<svg" in html_screenshot
    assert "reports/run-latest.html" in html_screenshot
