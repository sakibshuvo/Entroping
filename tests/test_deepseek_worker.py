...
def test_deepseek_worker_rejects_symlinked_artifact_root_before_artifacts(
    tmp_path: Path,
) -> None:
    repo = make_worker_repo(tmp_path)
    target_file = repo / "target.md"
...
def test_deepseek_worker_rejects_relative_artifact_root_escape(
    tmp_path: Path,
) -> None:
    repo = make_worker_repo(tmp_path)
    target_file = repo / "target.md"
...
def test_deepseek_worker_rejects_arbitrary_absolute_artifact_root_outside_repo_and_temp(
    tmp_path: Path,
) -> None:
    repo = make_worker_repo(tmp_path)
    target_file = repo / "target.md"
