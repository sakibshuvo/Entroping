...
def compose_counted_worker(
    observation: ScenarioObservation, decision: str, assignment_id: str | None
) -> None:
    if decision == "assigned":
        if assignment_id is None:
            raise AssertionError("assigned composition lacks an assignment identifier")
        observation.worker.dispatch(assignment_id)
    elif assignment_id is not None:
        raise AssertionError("blocked composition exposed an assignment identifier")
...
def finalize_receipt(
    ...
    invariants: list[InvariantClass] = ["offline", "no-provider", "no-source-mutation"]
...
def _validate_payload(payload: dict[str, object], encoded: str) -> None:
    ...
    if not isinstance(files, int) or not isinstance(bytes_total, int):
