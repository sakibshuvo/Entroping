...
from entroping.core.connector_intent import CONNECTOR_INTENT_SCHEMA_VERSION
from entroping.core.design_partner_feedback import DESIGN_PARTNER_FEEDBACK_SCHEMA_VERSION
from entroping.core.devex_readiness import DEVEX_READINESS_SCHEMA_VERSION
...
def _read_source_artifact_bytes(path: Path) -> tuple[bytes | None, str]:
    if _supports_no_follow_tree_open():
        return _read_source_artifact_bytes_no_follow(path)
    return _read_source_artifact_bytes_best_effort(path)
...
def _read_source_artifact_bytes_no_follow(path: Path) -> tuple[bytes | None, str]:
...
def _read_source_artifact_bytes_best_effort(path: Path) -> tuple[bytes | None, str]:
    file_descriptor: int | None = None
    try:
...
def _read_bounded_bytes_from_descriptor(file_descriptor: int) -> tuple[bytes | None, str]:
    ...
    if not stat.S_ISREG(descriptor_stat.st_mode):
        return None, "not a file"
    if descriptor_stat.st_size > _MAX_SOURCE_BYTES:
        return None, f"artifact exceeds {_MAX_SOURCE_BYTES} bytes"
    chunks: list[bytes] = []
    bytes_read = 0
    while bytes_read <= _MAX_SOURCE_BYTES:
        chunk = os.read(file_descriptor, min(65536, _MAX_SOURCE_BYTES + 1 - bytes_read))
        if not chunk:
            break
        chunks.append(chunk)
