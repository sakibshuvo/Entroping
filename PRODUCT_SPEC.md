...
| ID | Requirement | Acceptance Criteria |
| --- | --- | --- |
| INT-001 | Generate tests from OpenAPI | `architect build --new` creates valid Hurl tests for operations and schemas |
...
| EXE-009 | Provide TUI workflow | `entroping studio --env local` opens Textual/Rich mission control for local debugging |
| EXE-010 | Preserve execution reproducibility | All blocking CI behavior must be explainable from committed Hurl files, env data, effective QAnstitution, and Hurl output |
