...
def validate_regular(directory_fd: int, name: str) -> None:
...
def discard_initializing_file(directory_fd: int) -> None:
    if not entry_exists(directory_fd, INITIALIZING_NAME):
        return
    validate_regular(directory_fd, INITIALIZING_NAME)
...
def fsync_regular(directory_fd: int, name: str) -> None:
    ...
    try:
        metadata = os.fstat(descriptor)
        if not stat.S_ISREG(metadata.st_mode) or stat.S_IMODE(metadata.st_mode) != 0o600:
            raise FactoryBudgetLedgerError("path", "initializing ledger is unsafe")
        os.fsync(descriptor)
    finally:
        os.close(descriptor)
