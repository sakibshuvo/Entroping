"""Compatibility exports for factory metrics schema constants."""
# ruff: noqa: E501

from __future__ import annotations

from .context_schema import (
    CONTEXT_SCORECARD_ALLOWED_KEYS,
    CONTEXT_SCORECARD_ALLOWED_SOURCE_TYPES,
    CONTEXT_SCORECARD_BASELINE_KEYS,
    CONTEXT_SCORECARD_EVALUATION_KEYS,
    CONTEXT_SCORECARD_EVIDENCE_KEYS,
    CONTEXT_SCORECARD_FORBIDDEN_SOURCE_TYPES,
    CONTEXT_SCORECARD_HIGHER_IS_BETTER,
    CONTEXT_SCORECARD_INTEGER_METRICS,
    CONTEXT_SCORECARD_PROOF_STATUSES,
    CONTEXT_SCORECARD_RATE_METRICS,
    CONTEXT_SCORECARD_RECOMMENDATIONS,
    CONTEXT_SCORECARD_REQUIRED_BASELINE_COMPONENTS,
    CONTEXT_SCORECARD_REQUIRED_METRICS,
    CONTEXT_SCORECARD_SETUP_KEYS,
    CONTEXT_SCORECARD_SETUP_STATUSES,
    CONTEXT_SCORECARD_TRIAL_KEYS,
)
from .event_schema import (
    ALL_METRICS,
    ALLOWED_EVENT_KEYS,
    ALLOWED_METRIC_KEYS,
    CONTEXT_SCORECARD_REPORT_SCHEMA_VERSION,
    CONTEXT_SCORECARD_SCHEMA_VERSION,
    CONTROL_CHARACTER_PATTERN,
    DECISIONS,
    DEFAULT_LEDGER,
    EVENT_SCHEMA_VERSION,
    EVENT_TYPES,
    FINISHED_ISSUE_DIR_RE,
    FINISHED_ISSUES_DIR,
    FLOAT_METRICS,
    INTEGER_METRICS,
    NOTE_FORBIDDEN_PATTERN,
    NOTE_MAX_LENGTH,
    OUTCOMES,
    READINESS_SCHEMA_VERSION,
    REPORT_SCHEMA_VERSION,
    ROLES,
    SECRET_REDACTIONS,
    SUMMARY_SCHEMA_VERSION,
    TEXT_FIELDS,
    UTC_TZ,
)
from .readiness_schema import (
    NO_PROVIDER_MARKERS,
    QUALITY_MARKERS,
    READINESS_GATES,
    READINESS_MISSING_MESSAGES,
    SECURITY_MARKERS,
)

# Keep all imported constants explicitly referenced so linters and query checks
# treat this module as exporting active symbols, not a pure re-export shim.
_SCHEMA_CONSTANTS_REFERENCE = (
    CONTEXT_SCORECARD_ALLOWED_KEYS,
    CONTEXT_SCORECARD_ALLOWED_SOURCE_TYPES,
    CONTEXT_SCORECARD_BASELINE_KEYS,
    CONTEXT_SCORECARD_EVALUATION_KEYS,
    CONTEXT_SCORECARD_EVIDENCE_KEYS,
    CONTEXT_SCORECARD_FORBIDDEN_SOURCE_TYPES,
    CONTEXT_SCORECARD_HIGHER_IS_BETTER,
    CONTEXT_SCORECARD_INTEGER_METRICS,
    CONTEXT_SCORECARD_PROOF_STATUSES,
    CONTEXT_SCORECARD_RATE_METRICS,
    CONTEXT_SCORECARD_RECOMMENDATIONS,
    CONTEXT_SCORECARD_REQUIRED_BASELINE_COMPONENTS,
    CONTEXT_SCORECARD_REQUIRED_METRICS,
    CONTEXT_SCORECARD_SCHEMA_VERSION,
    CONTEXT_SCORECARD_REPORT_SCHEMA_VERSION,
    CONTEXT_SCORECARD_SETUP_KEYS,
    CONTEXT_SCORECARD_SETUP_STATUSES,
    CONTEXT_SCORECARD_TRIAL_KEYS,
    ALLOWED_EVENT_KEYS,
    ALLOWED_METRIC_KEYS,
    ALL_METRICS,
    CONTROL_CHARACTER_PATTERN,
    DECISIONS,
    DEFAULT_LEDGER,
    EVENT_SCHEMA_VERSION,
    EVENT_TYPES,
    FINISHED_ISSUE_DIR_RE,
    FINISHED_ISSUES_DIR,
    FLOAT_METRICS,
    INTEGER_METRICS,
    NOTE_FORBIDDEN_PATTERN,
    NOTE_MAX_LENGTH,
    OUTCOMES,
    READINESS_SCHEMA_VERSION,
    REPORT_SCHEMA_VERSION,
    ROLES,
    SECRET_REDACTIONS,
    SUMMARY_SCHEMA_VERSION,
    TEXT_FIELDS,
    UTC_TZ,
    NO_PROVIDER_MARKERS,
    QUALITY_MARKERS,
    READINESS_GATES,
    READINESS_MISSING_MESSAGES,
    SECURITY_MARKERS,
)

__all__ = [
    "CONTEXT_SCORECARD_ALLOWED_KEYS",
    "CONTEXT_SCORECARD_ALLOWED_SOURCE_TYPES",
    "CONTEXT_SCORECARD_BASELINE_KEYS",
    "CONTEXT_SCORECARD_EVALUATION_KEYS",
    "CONTEXT_SCORECARD_EVIDENCE_KEYS",
    "CONTEXT_SCORECARD_FORBIDDEN_SOURCE_TYPES",
    "CONTEXT_SCORECARD_HIGHER_IS_BETTER",
    "CONTEXT_SCORECARD_INTEGER_METRICS",
    "CONTEXT_SCORECARD_PROOF_STATUSES",
    "CONTEXT_SCORECARD_RATE_METRICS",
    "CONTEXT_SCORECARD_RECOMMENDATIONS",
    "CONTEXT_SCORECARD_REQUIRED_BASELINE_COMPONENTS",
    "CONTEXT_SCORECARD_REQUIRED_METRICS",
    "CONTEXT_SCORECARD_SCHEMA_VERSION",
    "CONTEXT_SCORECARD_REPORT_SCHEMA_VERSION",
    "CONTEXT_SCORECARD_SETUP_KEYS",
    "CONTEXT_SCORECARD_SETUP_STATUSES",
    "CONTEXT_SCORECARD_TRIAL_KEYS",
    "ALLOWED_EVENT_KEYS",
    "ALLOWED_METRIC_KEYS",
    "ALL_METRICS",
    "CONTROL_CHARACTER_PATTERN",
    "DECISIONS",
    "DEFAULT_LEDGER",
    "EVENT_SCHEMA_VERSION",
    "EVENT_TYPES",
    "FINISHED_ISSUE_DIR_RE",
    "FINISHED_ISSUES_DIR",
    "FLOAT_METRICS",
    "INTEGER_METRICS",
    "NOTE_FORBIDDEN_PATTERN",
    "NOTE_MAX_LENGTH",
    "OUTCOMES",
    "READINESS_SCHEMA_VERSION",
    "REPORT_SCHEMA_VERSION",
    "ROLES",
    "SECRET_REDACTIONS",
    "SUMMARY_SCHEMA_VERSION",
    "TEXT_FIELDS",
    "UTC_TZ",
    "NO_PROVIDER_MARKERS",
    "QUALITY_MARKERS",
    "READINESS_GATES",
    "READINESS_MISSING_MESSAGES",
    "SECURITY_MARKERS",
]
