...
for file_path, entry in data.items():
    ...
    if isinstance(entry, dict):
        rank = str(entry.get("rank", "")).upper()
        mi = entry.get("mi", entry.get("maintainability_index", "?"))
    elif isinstance(entry, list) and entry:
        first = entry[0]
        if isinstance(first, dict):
            rank = str(first.get("rank", "")).upper()
            mi = first.get("mi", first.get("maintainability_index", "?"))
    if rank and rank_order.get(rank, 0) > rank_order[worst_rank]:
        worst_rank = rank
...
uv run vulture src tests --min-confidence "${vulture_confidence}"
